...
struct statvfs getFileStatvfs(const char* path);
...
int main(int argc, char* argv[]) {
   ...
   struct statvfs svfs = getFileStatvfs(argv[1]);

   printf("Failo sistemos bloko dydis: %ld\n", (long) svfs.f_bsize);
   printf("Failo sistemos ID: %ld\n", (long) svfs.f_fsid);
   printf("Failo sistemos dydis: %ld\n", (long) sb.st_blksize);
